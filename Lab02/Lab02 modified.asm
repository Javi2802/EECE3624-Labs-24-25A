...
mainLoop:
    ...
    ; Delay for 40ms
    ldi R16, 2	              ; R16 is outer loop counter
outer_loop1:
    ldi R24, low(0x8000)        ; Inner loop count (0x8000 = 32,768 cycles)
    ldi R25, high(0x8000)       ; loop count by loading registers separately
    inner_loop1:
    ...
    ; Delay for 160ms
    ldi R16, 8                 ; R16 is outer loop counter
outer_loop2:
    ldi R24, low(0x8000)        ; Inner loop count (0x8000 = 32,768 cycles)
    ldi R25, high(0x8000)       ; loop count by loading registers separately
    inner_loop2:
